...
fwrite(summary_inVar, file = paste("./figures/tables/summary_", inVar, ".txt"), sep = ";")
...
nirs_herbivores$reindeer <- filter(nirs_herbivores$reindeer, 
                                   !nirs_herbivores$reindeer[,inVar] > 3.5) #Remove 3 samples due to high influence in the model (Y-Outliers)
...
#access results
geese_results$model_perf #variable of importance
geese_results$vips #variable of importance

## make calibration and validation plots
...
ggsave(paste0("./figures/pls_model/g_plot_nirs_results_", paste(inVar), ".png"), 
       g_plot_nirs, 
       width = 12, height = 10)
...
plot_cal <- ggplot(results_cal, aes(x = N, y = PLSR_Predicted)) + 
  # custom made theme
  theme_perso() + 
...
results_val <- list(
  ...
  # cleaning up model names and transform into factor
  mutate(model = str_replace(model, "_results$", "")) %>% 
  mutate(factor(model, levels = c("all","mammal", "geese", "sheep", "reindeer"))) 

# scatter plot observed vs predicted data for all model
plot_val <- ggplot(results_val, aes(x = N, y = PLSR_Predicted)) + 
  theme_perso() + 
  geom_point(aes(color = as.factor(session)), size = 5, alpha = 0.5) + 
  geom_smooth(size = 1, method = "lm", color = "#6A9C89") +  # Specify the color directly here
  # add the 95 confidence interval from the boostrapping
  geom_errorbar(aes(ymin = LCI, ymax = UCI, color = as.factor(session)), width = 0.1, alpha = 0.7) +
  geom_abline(intercept = 0, slope = 1, color = "dark grey", linetype = "dashed", size = 1) +
  labs(y = paste0("Predicted N (%)"),
       x = paste0("Observed N (%)"),
...
fwrite(summary_statistics_cal, 
       file = paste("./figures/tables/summary_statistics/summary_statistics_cal_",inVar, ".txt"), sep = ";")
fwrite(summary_statistics_val, 
       file = paste("./figures/tables/summary_statistics/summary_statistics_val_",inVar, ".txt"), sep = ";")
...
fwrite(model_perfs, 
       file = paste("./figures/tables/model_perf/model_perfs_",inVar, ".txt"), sep = ";")

#----------------------------------------------------------------------------###
### save model
#----------------------------------------------------------------------------###

nirs_faecal_N_model2024 <- all_results$model
save(nirs_faecal_N_model2024, 
     file = "./code/a_ready_to_apply_nirs_model_nutrient/nirs_faecal_N_model2024.rda")  # save the model
